...
import { AUTH_TYPE, ILOGINRESPONSE } from "../@types/index";
...
export const AuthenticationContextProvider = ({children}: {children: ReactNode})=>{
    ...
    const onLogin = async(payload: {email: string; password: string;}): Promise<unknown> => {
    ...
    return (
        <AuthenticationContext.Provider value={{user, loading, onLogin, onLogout}}>
            {children}
